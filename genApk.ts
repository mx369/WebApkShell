...
const getCfg = async () => {
    ...
    return webApkShell as Partial<{
        ...
        AP_VERSION_NAME: ''
        AP_SPLASH_TIMEOUT: '2000'
    }>
}

const { build_script, dist_path, AP_ICON, AP_SPLASH_TIMEOUT, ...APS } = await getCfg()
...
const main = async () => {
    ...
    // 生成编译期常量文件，值在打包时就固定了
    await writeFile(
        './WebApkShell/app/src/main/java/com/qcp/androidshell/ShellConfig.kt',
        `package com.qcp.androidshell\n\nobject ShellConfig {\n    const val SPLASH_TIMEOUT = ${Number(AP_SPLASH_TIMEOUT) || 2000}L\n}\n`
    )
    await gradlew(APS)
}
main()
